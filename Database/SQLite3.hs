...
sqlError maybeDatabase functionName error = do
  details <- case maybeDatabase of
               Just database -> do
                 ...
                 return $ ": " ++ details
               Nothing -> return "."
  throw $ ErrorCall $ "###SQLite3 returned " ++ (show error)
...
prepareMError :: Database -> String -> IO (Either [Statement] Error)
prepareMError (Database db) text = do
  BS.useAsCString (UTF8.fromString text) (\s'->alloca (\st->alloca (\s->(poke s s'>>m st s))))
  where
  m::Ptr (Ptr ())->(Ptr (Ptr CChar))->IO (Either [Statement] Error)
  m st s = do
    --(peek s>>=print)>>hFlush stdout
    e<-peek s>>=(\s'->prepareC db s' (-1) st (castPtr s))
    case decodeError e of
      ErrorOK->do 
	st'<-peek st
	if st'==nullPtr 
	then return $ Left []
	  ...
	  r<-m st s
	  return (case r of
	    Left sts -> Left $ (Statement st'):sts
	    rt -> rt)
      e' -> return $ Right e'
prepareM :: Database -> String -> IO [Statement]
prepareM database text = prepareMError database text>>=(\r->
  case r of
    Left ss -> return ss
    Right error -> sqlError (Just database) ("prepareM " ++ (show text)) error)

runScript :: Database -> String -> [SQLData] -> IO [[[SQLData]]]
runScript (Database db) scr bs = do
  BS.useAsCString (UTF8.fromString scr) (\s'->alloca (\st->alloca (\s->(poke s s'>>m st s))))
  where
  m::Ptr (Ptr ())->(Ptr (Ptr CChar))->IO [[[SQLData]]]
  m st s = do
    e<-peek s>>=(\s'->prepareC db s' (-1) st (castPtr s))
    case decodeError e of
      ErrorOK->do
        ...
        then return []
          ...
          r<-sqlite3_bind_parameter_count st'>>=run (Statement st')
          m st s>>=return.(r:)
      e'->sqlError (Just (Database db)) ("runScript " ++ show scr) e'
  run stmt n = bind stmt (take n bs)>>step stmt>>=d where
    d Row = do
      r<-columns stmt
      rs<-step stmt>>=d
      return (r:rs)
...
step statement = do
  ...
  case error of
    ...
    ErrorBusy -> threadDelay 10000>>step statement
